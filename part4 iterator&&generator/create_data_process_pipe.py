# 管道的方式迭代处理数据，比如有一个很大的数据处理，但是不能将它们一次性放入内存。

# 生成器函数是一个实现管道机制的好方法。为了演示，假定你要处理一个非常大的日志文件目录：
...
def gen_find(filepat, top):
    '''
    Find add filenames in a directory tree that match a shell wildcard pattern
    '''
