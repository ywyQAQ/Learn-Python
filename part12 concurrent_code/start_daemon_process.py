# 想编写一个作为一个在Unix系统上运行的守护进程运行的程序。

# 创建一个守护进程需要一个精准的系统调用序列以及对于细节的控制。
# 下面的代码展示了怎样定义一个守护进程，可以启动后很容易地停止它。
...
def daemonize(pidfile, *, stdin='/dev/null',
    ...
    try:
        ...
        raise RuntimeError('fort #1 failed')
...
# sys.stdout, sys.__stdout__等）。 仅仅简单的关闭 sys.stdout 并重新指定它是行不通的， 
...
# 信号处理器简单的抛出了 SystemExit() 异常。 或许这一步看上去没必要，但是没有它， 
