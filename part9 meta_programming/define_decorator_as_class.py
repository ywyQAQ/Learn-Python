...
# 例如，下面的代码定义了一个类，它在其它函数上放置一个简单的记录层。
...
# 将装饰器定义成类通常是很简单的。但是这里还是有一些细节需要解释下，
# 特别是当你想将它作用在实例方法上的时候。[在类里]

# 首先，使用 functools.wraps() 函数的作用跟之前还是一样，
# 将被包装函数的元信息复制到可调用实例中去。

# 其次，通常很容易会忽视上面的 __get__() 方法。如果你忽略它，
# 保持其他代码不变再次运行， 你会发现当你去调用被装饰实例方法时出现很奇怪的问题。
# 例如：
...
# 在8.9小节已经讲述过描述器协议了。在这里，__get__() 的目的是创建一个绑定方法对象 (最终会给这个方法传递self参数)。
# 下面是一个例子来演示底层原理：
s = Spam()
def grok(self, x):
    pass
...
# 这个方式跟之前的效果几乎一样，
# 除了对于 ncalls 的访问现在是通过一个被绑定为属性的函数来实现，例如：
add(2, 3)
# 5
add(4, 5)
# 9
