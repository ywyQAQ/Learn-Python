# 自动记录一个类中的属性和方法定义的顺序，然后可以利用它来做很多操作（比如序列化、映射到数据库等等）

# 利用元类可以很容易捕获到类的定义信息。下面是一个例子，使用一个OrderedDict来记录描述器的定义顺序：
...
# 生成的有序名称从字典中提取出来并放入类属性 _order 中。
...
# 在框架底层，我们必须捕获定义的顺序来将对象映射到元组或数据库表中的行
# （就类似于上面例子中的 as_csv() 的功能）。 
# 这节演示的技术非常简单，并且通常会比其他类似方法
# （通常都要在描述器类中维护一个隐藏的计数器）要简单的多。