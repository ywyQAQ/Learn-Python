# 写一段代码，最终要创建一个新的类对象。你考虑将类的定义源代码以字符串的
# 形式发布出去。并且使用函数比如exec()来执行它，但是你想寻找一个更加优雅的解决方案。

# 你可以使用函数types.new_class()来初始化新的类对象。你需要做的只是提供类的名字
# 父类元祖、关键字参数，以及一个用成员变量填充类字典的回调函数。例如：
...
# 这种方法中，一个比较难理解的地方是在调用完types.new_class()对Stock.__module__
# 的赋值，每次当一个类被定义后，它的__module__属性包含定义它的模块名。这个名字用于生成
# __repr__()的输出。它同样用于很多库，比如pickle。因此，为了让你创建的类是正确的。
# 你需要确保这个属性也设置正确了。
